...
from glideinwms.lib.defaults import BASE_DIR, KEY_DIR, WEB_SEC_DIR
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-k",
        "--private-key-file",
        help="Location of the unencrypted private key file. Will be created if it does not exist.",
        default=os.path.join(KEY_DIR, "frontend_scitokens_key.pem"),
    )
    parser.add_argument(
        "-e",
        "--ec",
        help="Use elliptical curve cryptography. Defaults to False or whatever is the type of the existing key.",
        action="store_true",
    )
    ...
    parser.add_argument(
        ...
        default=os.path.join(BASE_DIR, "vofrontend"),
    )
    parser.add_argument(
        "-w",
        "--web-dir",
        ...
        default=os.path.join(WEB_SEC_DIR, "scitokens"),
    )
    parser.add_argument(
        "-u", "--web-url", help="Base URL for web server. Defaults to the /cred/scitokens URL in the Frontend."
    )
    parser.add_argument(
        "-g", "--group", help="Optional key group. By default will use the global key, not specific to any group."
    )
    return parser.parse_args()


def make_key(private_key_file, use_ec=False, force_new=False) -> types.PrivateKeyTypes:
    ...
    Args:
        ...
        use_ec (bool): If True, use EC private key instead of RSA private key.
        force_new (bool): If True, overwrite existing private key.
...
def get_jwk_pk_string(private_key, key_type) -> str:
    ...
    Returns:
        str: multiline JSON formatted string with serialized JWK public key.
    """
    ...
    log(f"Returning {key_type} JWK public key, kid: {kid}")
    return json.dumps(jwk_public_key, sort_keys=True, indent=4, separators=(",", ": "))


def prepare_web_dir(web_dir, group_name, uri, public_key) -> str:
    ...
    Returns:
        str: Public key advertising URI.
    """
...
def main():
    ...
    gfe_desc = glideinFrontendConfig.ElementMergedDescript(args.work_dir, args.group)
    # Calculate and verify consistency of private key
    private_key = make_key(args.private_key_file, args.use_ec)
    if isinstance(private_key, rsa.RSAPrivateKey):
        key_type = "RSA"
    elif isinstance(private_key, ec.EllipticCurvePrivateKey):
        key_type = "EC"
    else:
        raise ValueError("The private key is of an unknown type (not RSA or EC).")
    if args.use_ec and not key_type == "EC":
        raise ValueError("Private key type mismatch. Requested EC, but loaded RSA.")
    # Calculate and publish public key
    pk_str = get_jwk_pk_string(private_key, key_type)
    uri = args.web_url or os.path.join(os.path.dirname(gfe_desc.frontend_data["MonitoringWebURL"]), "cred", "scitokens")
    adv_uri = prepare_web_dir(args.web_dir, args.group, uri, pk_str)
    log(f"Public key advertised at: {adv_uri}")
